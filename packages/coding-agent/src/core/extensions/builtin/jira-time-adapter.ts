...
export async function readTimingSessions(root: string): Promise<Timing[]> {
	...
	try {
		...
		for (const name of files) {
			let lines: string[];
			try {
				lines = (await readFile(join(root, name), "utf8")).split("\n").filter(Boolean);
			} catch {
				continue;
			}
			for (const line of lines) {
				try {
					const r = JSON.parse(line) as { event?: string; session_id?: string; duration_ms?: number };
					if (r.event === "session" && r.session_id)
						totals.set(r.session_id, (totals.get(r.session_id) ?? 0) + Number(r.duration_ms ?? 0));
				} catch {
					/* skip malformed timing rows */
				}
			}
		}
	} catch {
		/* timing extension is optional */
	}
...
export async function summarizeTiming(root: string, jiraRows: Row[]): Promise<string> {
	...
	const joined = jiraRows
		.filter((r) => r.primeSessionId)
		.map((r) => {
